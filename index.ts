...
function getCTokens(): any {
    return new Promise((resolve, reject) => {
        https.get("https://api.compound.finance/api/v2/ctoken", (resp) => {
            ...
            resp.on('end', () => {
                var decoded = JSON.parse(data);
                if (!decoded || !decoded.cToken) return reject("Failed to decode cToken list from Compound API");
...
function getMarketHistory(tokenAddress, minTimestamp, maxTimestamp, buckets): any {
    return new Promise((resolve, reject) => {
        https.get("https://api.compound.finance/api/v2/market_history/graph?asset=" + tokenAddress + "&min_block_timestamp=" + minTimestamp.toString() + "&max_block_timestamp=" + maxTimestamp.toString() + "&num_buckets=" + buckets.toString(), (resp) => {
            ...
            resp.on('end', () => {
                var decoded = JSON.parse(data);
                if (!decoded) return reject("Failed to decode market history from Compound API");
                resolve(decoded);
            });
        }).on("error", (err) => {
...
async function updateAllCompoundAprs() {
    // Get cTokens
    const cTokens = await getCTokens();

    // Get cToken USD prices
    var currencyCodes = ["COMP"];
    for (const cToken of cTokens) currencyCodes.push(cToken.underlying_symbol);
    var prices = await getCurrencyUsdRates(currencyCodes); // TODO: Get real USD prices, not DAI prices
    ...
    for (const cToken of cTokens) {
        const history = await getMarketHistory(cToken.token_address, epochOneYearAgo, epoch, 365);
    ...
    try {
        await fs.writeFile(process.env.COMPOUND_APRS_SAVE_PATH, JSON.stringify(compoundApys));
    } catch (error) {
